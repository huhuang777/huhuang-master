...
global.console = Object.assign(console, {
  ...
  warn: (...args) =>
    warn(color('\x1b[33m%s\x1b[0m'), '[warn]', '[huhuang.net]', ...args),
  info: (...args) =>
    info(color('\x1b[34m%s\x1b[0m'), '[info]', '[huhuang.net]', ...args),
  error: (...args) =>
    info(color('\x1b[31m%s\x1b[0m'), '[error]', '[huhuang.net]', ...args)
})
...
if (config.dev) {
  const handleProxy = path => (req, res) => {
    const targetUrl =
      'http://' + req.url.replace('/proxy/' + (path ? path + '/' : ''), '')
    require('request')
      .get(targetUrl)
      .pipe(res)
  }
...
const bootstrap = () => {
  ...
  console.info(
    `${appName} ${envText}启动成功！listening on ${host}:${port}, at ${new Date().toLocaleString()}`
  )
  // 启动扩展服务
  updateGAScript()
}
