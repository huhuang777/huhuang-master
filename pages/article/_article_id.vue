      ...
      <transition name="module" mode="out-in">
        <div class="readmore" v-if="canReadMore">
          <button class="readmore-btn" :disabled="readMoreLoading" @click="readMore()">
            <span>{{ !readMoreLoading ? '阅读全文' : '渲染中...'}}</span>
        ...
        <div class="article-list swiper" v-swiper:swiper="swiperOption">
          <div class="swiper-wrapper">
            <div class="swiper-slide item" v-for="(article, index) in article.related" :key="index">
              <nuxt-link :to="`/article/${article.id}`" :title="article.title" class="item-box">
  ...
  methods: {
    ...
    copyArticleUrl() {
      if (this.article.title) {
        // console.log('要复制了', `https://huhuang.net/article/${this.article.id}`, this.$root.$copyToClipboard)
        this.$root.$copyToClipboard(
          `https://huhuang.net/article/${this.article.id}`
        )
      }
    },
    buildThumb(thumb) {
      if (!thumb) return `${this.cdnUrl}/images/thumb-releted.jpg`
      return `${thumb}?imageView2/1/w/300/h/230/format/${
        this.imgExt
      }/interlace/1/q/80|imageslim`
    },
    buildDateTitle(date) {
      if (!date) return date
      date = new Date(date)
